...
export const SettingsPage: React.FC = () => {
  ...
  const { user, profile, updateProfile, uploadAvatar, logout } = useAuthStore();
  ...
  const handleAvatarFileChange = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (file.size > 10 * 1024 * 1024) {
      toast.error('Avatar size must be less than 10MB');
      return;
    }
      ...
      <div className="flex-1 overflow-y-auto p-4 sm:p-8 max-w-4xl mx-auto w-full space-y-6">
        ...
        {activeTab === 'profile' && (
          <form onSubmit={handleSaveProfile} className="space-y-6">
            {/* Avatar Header Preview & Direct Cloudinary Upload */}
            <div className="p-6 rounded-3xl bg-slate-900/60 border border-slate-800 flex flex-col sm:flex-row items-center gap-6">
              ...
              >
                <Avatar
                  ...
                  status="ONLINE"
                />
                <div className="absolute inset-0 rounded-full bg-slate-950/70 opacity-0 group-hover:opacity-100 flex flex-col items-center justify-center text-white transition-opacity duration-150">
              ...
              <div className="flex-1 text-center sm:text-left space-y-1">
                ...
                <div className="pt-2 flex flex-wrap items-center justify-center sm:justify-start gap-2.5">
                  ...
                  >
                    ...
                    <span>{isUploadingAvatar ? 'Uploading to Cloudinary...' : 'Upload Photo'}</span>
                  </Button>
                  ...
                  {avatarUrl && (
                    <button
                      ...
                      className="text-xs text-slate-400 hover:text-rose-400 font-medium px-2 py-1 transition-colors cursor-pointer"
                    >
                      Remove
                    </button>
                  )}
                ...
                <p className="text-[11px] text-slate-500 pt-1">
                  Upload directly to Cloudinary (JPEG, PNG, GIF, WebP up to 10MB).
                </p>
              </div>
            </div>
